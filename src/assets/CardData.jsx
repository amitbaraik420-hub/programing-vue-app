...
export default function CardData({Prom}) {
    const {picture,name,status } = Prom
    // console.log(hero)
  ...
  <div className="card-body ">
    <h2 className=" text-center text-2xl font-bold">{name}</h2>
      ...
      <p className='text-center text-green-800'>Femail</p>
    <div className=" flex justify-between">
        
         <Link to={`/chemistry/${Prom.id}`}>
               <button className="btn btn-primary text-red-300 rounded-4xl relative mx-22">{status}</button>
         </Link>
